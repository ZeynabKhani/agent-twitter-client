...
async function fetchRetweetersPage(tweetId, auth, cursor, count = 40) {
  const baseUrl = "https://twitter.com/i/api/graphql/6zT6vBAmxdwb4nb4lBdSFw/Retweeters";
  const variables = {
    tweetId,
    count,
    cursor,
...
async function fetchLikersPage(tweetId, auth, cursor, count = 40) {
  const baseUrl = "https://twitter.com/i/api/graphql/DlURTJjCVitiSKrykYTkag/Favoriters";
  const variables = {
    tweetId,
    count,
    cursor,
...
async function getAllLikers(tweetId, auth) {
  let allLikers = [];
  let cursor;
  while (true) {
...
class Scraper {
   ...
   */
  async getAllLikersOfTweet(tweetId) {
...
export { ChatClient, HlsRecordPlugin, IdleMonitorPlugin, JanusAudioSink, JanusAudioSource, JanusClient, Logger, MonitorAudioPlugin, RecordToDiskPlugin, Scraper, SearchMode, Space, SpaceParticipant, SttTtsPlugin, fetchLikedTweets, getAllLikers };
